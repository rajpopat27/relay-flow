...
step="$1"
mkdir -p "$GIF_DIR"
asciinema rec -f asciicast-v2 --overwrite --cols 110 --rows 32 -c "bash $HERE/$step.sh" "$GIF_DIR/$step.cast" </dev/null
agg --speed 1.5 "$GIF_DIR/$step.cast" "$GIF_DIR/$step.gif"
