    ...
    event: async ({ event: rawEvent }) => {
      ...
      try {
        ...
        if (event.type === "session.idle") {
          ...
          if (
            aborted ||
            handledAssistantIDs.has(info.id) ||
            inFlightHITLCorrectionIDs.has(info.id)
          ) return;
          ...
          if (ctx.nodeType === "hitl") {
            ...
            if (outcome.kind === "approve") {
              debug("hitl output awaiting tui approval", attrs);
              return;
            }
            if (outcome.kind === "silent") {
              debug("hitl output silent", attrs);
              return;
            }
            operation = "hitl-correction";
            inFlightHITLCorrectionIDs.add(info.id);
            try {
              await client.session.promptAsync({
                path: { id: sessionID },
                body: { parts: [{ type: "text", text: INVALID_REPORT_PROMPT }] },
              });
              handledAssistantIDs.add(info.id);
            } finally {
              inFlightHITLCorrectionIDs.delete(info.id);
            }
            debug("hitl invalid output corrected", attrs);
            return;
          }
