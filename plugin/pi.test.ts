...
describe("Pi HITL direct UI contract", () => {
  test("missing and empty output stay silent", async () => {
    for (const branch of [
      [],
      [assistantEntry("empty-assistant-entry", [{ type: "text", text: "   " }])],
    ]) {
      const fixture = relayFixture();
      ...
      context.ui.select = async (...args: unknown[]) => {
        ...
        throw new Error("HITL UI must stay silent for missing output");
      };
  ...
  test("a corrected valid report opens the approval selector", async () => {
    const fixture = relayFixture();
    configureMetadata(fixture.directory, `run-${fixture.directory.split("/").pop()}`, "hitl");
    const pi = makePi();
    ...
    branch.push(assistantEntry("corrected-report-entry", [{ type: "text", text: validReport }]));
    await handler(pi, "agent_settled")(settled(), context);
  ...
  test("aborted output stays silent even when non-empty and invalid", async () => {
    ...
    const pi = makePi();
    const context = makeContext(
      "pi-session-hitl-aborted",
      [assistantEntry("aborted-hitl-entry", [{ type: "text", text: "review complete" }], "aborted")],
    );
    context.ui.select = async () => {
      throw new Error("HITL UI must stay silent for aborted output");
    };
