...
// message text to `relay-flow report`, a thin socket client that asks the
// running relay-flow server to record the outcome in the tracker. The plugin
// never closes its own terminal; terminals are cleaned up by the daemon
// at closeOn nodes. Ticket/node/agent come from RELAY_FLOW_* env vars the
...
const logDir = join(homedir(), ".relay-flow")
...
export const ReportStatusPlugin: Plugin = async ({ client }) => {
  const workflowName = process.env.RELAY_FLOW_WORKFLOW
  const ticket = process.env.RELAY_FLOW_TICKET
  const node = process.env.RELAY_FLOW_NODE
  const agent = process.env.RELAY_FLOW_AGENT
    ...
    event: async ({ event }: { event: any }) => {
      ...
      if (event?.type === "session.idle") {
        ...
        if (!ticket || !agent || !node || !sessionID) {
          log("[report-status] RELAY_FLOW_TICKET/_NODE/_AGENT or sessionID missing, skipping report")
          return
        }
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
          try {
            const stdout = await execFileText("relay-flow", [
              "report",
            ...
            log(`[report-status] relay-flow report failed (attempt ${attempt + 1}/3):`, err)
          }
