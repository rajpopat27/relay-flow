...
describe("OpenCode server plugin", () => {
  ...
  test("partial report-shaped HITL output receives exactly one correction", async () => {
    ...
    await hooks.event!({ event: { type: "session.idle", properties: { sessionID: "hitl-session" } } } as any);
    // A duplicate idle event for the same assistant message must not nudge again.
    await hooks.event!({ event: { type: "session.idle", properties: { sessionID: "hitl-session" } } } as any);

    expect(prompts).toEqual([INVALID_REPORT_PROMPT]);
    expect(calls(f.calls).filter((call) => call.command === "report")).toHaveLength(0);
    expect(readFileSync(join(f.directory, "plugin.log"), "utf8")).toContain("hitl invalid output corrected");
  });

  test("concurrent duplicate invalid HITL events send one correction", async () => {
    const f = fixture();
    setEnvelope(f.directory, "hitl");
    const prompts: string[] = [];
    ...
    await hooks.event!({ event: { type: "session.created", properties: { info: { id: "hitl-session" } } } } as any);

    const idleEvent = { event: { type: "session.idle", properties: { sessionID: "hitl-session" } } } as any;
    const first = hooks.event!(idleEvent);
    await started;
    const second = hooks.event!(idleEvent);
    try {
      // The first prompt remains blocked; the duplicate handler must return
      // without entering the second prompt call.
  ...
  test("missing HITL output remains silent", async () => {
    const f = fixture();
    setEnvelope(f.directory, "hitl");
    const prompts: string[] = [];
    const hooks = await RelayFlowPlugin({ client: { session: {
      update: async () => {},
      messages: async () => ({ data: [{
        info: { id: "empty-hitl", role: "assistant", time: { completed: Date.now() } },
    ...
    expect(prompts).toHaveLength(0);
    expect(calls(f.calls).filter((call) => call.command === "report")).toHaveLength(0);
    expect(readFileSync(join(f.directory, "plugin.log"), "utf8")).toContain("hitl output silent");
  });

  test("aborted HITL output remains silent", async () => {
    const f = fixture();
    setEnvelope(f.directory, "hitl");
    const prompts: string[] = [];
    const hooks = await RelayFlowPlugin({ client: { session: {
      update: async () => {},
      messages: async () => ({ data: [{
        info: { id: "aborted-hitl", role: "assistant", time: { completed: null } },
