...
- Pi has no OpenCode-style `--agent`, `--auto`, or `mode: primary` options. Worker identity comes from the native prompt template passed with `--prompt-template`.
...
- Implementer:
  `orca-ide terminal create --worktree current --title "implementer-s<N>" --command "pi --name implementer-s<N> --model github-copilot/gpt-5.6-luna --thinking max --prompt-template .pi/prompts/implementer.md" --json`
- Verifier:
  `orca-ide terminal create --worktree current --title "verifier-s<N>" --command "pi --name verifier-s<N> --model github-copilot/gpt-5.6-sol --thinking high --exclude-tools edit,write --prompt-template .pi/prompts/verifier.md" --json`
