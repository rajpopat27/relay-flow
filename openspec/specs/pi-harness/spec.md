...
Pi node commands SHALL launch the default interactive TUI. The command SHALL NOT use print mode, JSON mode, or RPC mode. The runner terminal SHALL provide a TTY for both standard streams. When a project prompt template is selected, the command SHALL load that template with `--prompt-template` before the final positional prompt.
...
#### Scenario: Fresh prompt-template Pi node launch

- **WHEN** a node with `agent: coder` has no persisted session ID and the registered repository contains `.pi/prompts/coder.md`
- **THEN** the command includes `--prompt-template .pi/prompts/coder.md`, uses `--name <ticket>:<node>`, and supplies `/coder <rendered prompt>` as the final positional argument
...
#### Scenario: Resumed prompt-template Pi node launch

- **WHEN** a node with `agent: reviewer` has persisted session ID `session-123` and the registered repository contains `.pi/prompts/reviewer.md`
- **THEN** the command includes `--prompt-template .pi/prompts/reviewer.md`, includes `--session-id session-123`, and supplies `/reviewer <rendered prompt>` as the final positional argument without a bare `--` terminator
...
### Requirement: Pi uses user-owned agent configuration and prompt templates

The Pi harness SHALL use Pi's single built-in coding agent with the user's normal Pi model, provider, tools, extensions, and settings. The workflow `agent` value `default` SHALL select that built-in agent. A non-default workflow agent value SHALL select a project-owned prompt template at `.pi/prompts/<agent>.md` under the registered repository path; the harness SHALL pass that path to Pi with `--prompt-template` and submit the rendered relay-flow prompt as `/agent <arguments>`. Agent values SHALL NOT be interpreted as model IDs or OpenCode-style `--agent` options. Pi owns prompt-template discovery and expansion; relay-flow SHALL NOT maintain a named-agent registry or validate template file contents.
...
#### Scenario: Existing prompt template

- **WHEN** a Pi workflow uses `agent: coder` and the registered repository contains `.pi/prompts/coder.md`
- **THEN** validation succeeds and the launch command passes that template path with `--prompt-template` and sends `/coder <rendered prompt>`

#### Scenario: Missing prompt template

- **WHEN** a Pi workflow uses a non-default agent and `.pi/prompts/<agent>.md` is absent, empty, non-regular, or unreadable
- **THEN** Pi's prompt-template loader handles the template resource; relay-flow validation does not inspect the file before launch

#### Scenario: Unsafe prompt-template value

- **WHEN** a Pi workflow agent value contains path separators or path traversal
- **THEN** validation fails without resolving a file outside `.pi/prompts`
...
- **WHEN** a valid default agent or prompt template is validated and the real `pi` executable cannot be found on `PATH`
