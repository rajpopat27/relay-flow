# workflow-repo-management Specification
...
`relay-flow init` SHALL select and store task, runner, and harness plugin names and initialize SQLite. Its interactive titles SHALL be `Select task system`, `Select runner`, and `Select harness`; a plugin type with one registered option SHALL be selected automatically. On a new interactive first-run, the selected task plugin's existing authentication flow SHALL run inline after plugin selection and before the final machine config or durable state is committed. When Jira is selected, a following `Configure Jira` section SHALL collect Jira site, email, and a masked API token and validate them through Jira REST API v3 before writing state. The site SHALL be stored in machine task config; email and token SHALL be stored separately in owner-only `credentials.yaml`, and the token SHALL never be logged. Beads authentication SHALL remain a no-op owned by the Beads workspace/Dolt setup and SHALL not create relay-flow credentials. Equivalent non-interactive flags SHALL be available and SHALL not invoke hidden prompts or start a server. Initialization SHALL print the selected values and `Relay-flow initialized`. Without `--force`, existing machine config or database SHALL cause refusal. `init --force` SHALL refuse while the server is running or any run is nonterminal, and otherwise SHALL preserve `state.db`, completed history, workflows, logs, repo registrations, and unrelated machine settings while updating plugin selections and supplied Jira credentials. Interactive first-run SHALL ask whether to register repositories after successful initialization; choosing no SHALL leave the server stopped, while choosing yes SHALL show a Huh spinner while starting or reusing the background server, wait for the Unix socket to respond, and invoke the existing interactive repository-registration flow. A successful guided registration SHALL leave the server running. Initialization SHALL remain valid without repo registration.
...
#### Scenario: Interactive first-run skips repositories
- **WHEN** an interactive first-run completes authentication and the user declines repository setup
- **THEN** initialization succeeds without starting relay-flow's server, and repositories remain available for the standalone `repo register` command

#### Scenario: Interactive first-run registers repositories
- **WHEN** an interactive first-run completes authentication and the user chooses repository setup
- **THEN** relay-flow shows a Huh spinner, waits for a responsive Unix socket before discovery, reuses an already-running server, invokes the existing multi-repository registration flow, and leaves the ready server running
...
#### Scenario: Interactive authentication is canceled
- **WHEN** the user cancels plugin selection or the selected task authentication form during a new interactive first-run
- **THEN** initialization stops without leaving a partial machine config, task credentials file, or durable database
