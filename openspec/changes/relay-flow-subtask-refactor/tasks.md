...
- [ ] 2.2 Pin `github.com/cschleiden/go-workflows` to exactly `v1.4.2` and its embedded `modernc.org/sqlite` backend. Do not add an adapter against unpinned APIs. Also add `github.com/google/renameio/v2` and `github.com/charmbracelet/huh`; add no merge, CLI-framework, ORM, validation, UUID, DI, or state-machine libraries.
