...
- [x] 2.1 Upgrade the Go toolchain to `1.24.6` in `go.mod` and CI/release config; verify `go build` and release packaging work on the new toolchain. (Note: a `toolchain go1.24.6` line is intentionally omitted — `go mod tidy` strips it as redundant when the toolchain equals the `go` directive, and CI pins the version via `setup-go`.)
...
- [x] 2.3 Complete the go-workflows compatibility spike before the main rewrite: a small throwaway program/test covering SQLite startup, explicit instance IDs, duplicate start handling, separate Workflow/Activity workers, signals, durable timers, disconnected-context cancellation cleanup, and history inspection. Confirm each behavior passes, then delete the spike code — no written record is kept. If v1.4.2 fails any required behavior, stop and revise the design; do not silently switch versions.
