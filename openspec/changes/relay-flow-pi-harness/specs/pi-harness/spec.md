...
Pi node commands SHALL launch the default interactive TUI. The command SHALL NOT use print mode, JSON mode, or RPC mode. The runner terminal SHALL provide a TTY for both standard streams.
...
The Pi harness SHALL use Pi's single built-in coding agent with the user's normal Pi model, provider, tools, extensions, and settings. The workflow `agent` field SHALL use the logical value `default` and SHALL NOT be interpreted as a Pi model ID or OpenCode-style `--agent` option. `ValidateAgent` SHALL check only the `default` logical value and whether the real `pi` executable is discoverable on `PATH`; it SHALL not call a nonexistent agent-list command or validate a model by inventing a Pi agent API.
...
#### Scenario: Unsupported agent label

- **WHEN** a Pi workflow node uses an agent value other than `default`
- **THEN** Pi harness validation rejects it because Pi has no corresponding named agent

#### Scenario: Pi executable is unavailable

- **WHEN** `agent: default` is validated and the real `pi` executable cannot be found on `PATH`
