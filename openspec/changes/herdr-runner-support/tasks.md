...
- [ ] 1.3 Define the Herdr adapter construction contract in `design.md` before writing adapter behavior tests: the adapter-owned `Config` type and fields, strict raw-config decoding, the production constructor/factory signature, the test construction signature, and the exact package/test boundary.
- [ ] 1.4 Choose and document whether the test construction path is an unexported `newAdapter(cli herdrclicli.Client, cfg Config)` helper or another explicitly justified shape. The chosen contract SHALL be intentional and SHALL NOT be inferred by adapter tests from the Orca implementation.
...
- [ ] 2.4 After Tasks 1.3–1.5 define the adapter construction contract, add adapter tests for workspace discovery, normalized path matching, label tie-breaking, ambiguous workspace rejection, workspace reuse, missing-workspace errors, and concurrent lookup idempotence.
- [ ] 2.5 Using the construction contract from Tasks 1.3–1.5, add adapter tests proving `runner.Terminal.ID` is the public `pane_id`, `terminal_id` changes are ignored, exact `<ticket>:<node>` labels are used, and node/workflow/agent/nodeVisitID metadata never enters the label.
