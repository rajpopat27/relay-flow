...
export JIRA_PROJECT=DEMO        # a project you own
...
```json
{"id":"160248","key":"DEMO-41415","self":"…/rest/api/3/issue/160248"}
```

```bash
export TICKET=DEMO-41415
```
...
```text
herdr-e2e/herdrE2eFlow/DEMO-41415	DEMO-41415	herdrE2eFlow	starting
```
...
```json
{
  "id": "herdr-e2e/herdrE2eFlow/DEMO-41415",
  "repo": "herdr-e2e",
  "workflow": "herdrE2eFlow",
  "ticket": { "id": "160248", "key": "DEMO-41415", "title": "" },
  "state": "waiting",
...
ws w2 DEMO-41415 | /home/raj/.herdr/worktrees/herdr-e2e/demo-41415 | linked: True
tab w1:t1 '1' w1
tab w2:t1 '1' w2
tab w2:t2 'DEMO-41415:implement' w2
pane w1:p1 None /tmp/relay-e2e-XXXXXX/repos/herdr-e2e
pane w2:p1 None /home/raj/.herdr/worktrees/herdr-e2e/demo-41415
pane w2:p2 'DEMO-41415:implement' /home/raj/.herdr/worktrees/herdr-e2e/demo-41415
```
...
DEMO-41415 /home/raj/.herdr/worktrees/herdr-e2e/demo-41415 open_ws: w2
```
...
{"result":{"process_info":{
  ...
  "foreground_processes": [{
    "argv": ["opencode","--agent","build","--prompt","Task system: jira\nUse the jira tools to read the parent ticket DEMO-41415.\n\nYour mailbox is DEMO-41416. …"],
    "cwd": "/home/raj/.herdr/worktrees/herdr-e2e/demo-41415",
...
parent: In Progress labels: ['wf:herdrE2eFlow']
  DEMO-41416 DEMO-41415:implement -> In Progress
  DEMO-41417 DEMO-41415:verify -> To Do
```
...
```text
t+20s  run=[waiting implement]  panes=[DEMO-41415:implement]
t+40s  run=[running implement]  panes=[DEMO-41415:implement]
t+60s  run=[waiting implement]  panes=[DEMO-41415:implement]
…
t+140s run=[waiting verify]     panes=[DEMO-41415:implement,DEMO-41415:verify]
…
...
parent: Done labels: ['wf:herdrE2eFlow']
  DEMO-41416 DEMO-41415:implement -> Done
  DEMO-41417 DEMO-41415:verify -> Done
```
...
```bash
curl -s -u "$EMAIL:$TOKEN" "$SITE/rest/api/3/issue/DEMO-41417/comment" | python3 -c '
import json,sys
def flat(n):
...
```text
---
Feedback from implement to verify mailbox DEMO-41417
COMMITS: d22db8a
...
```text
main /tmp/relay-e2e-XXXXXX/repos/herdr-e2e open_ws: w1
DEMO-41415 /home/raj/.herdr/worktrees/herdr-e2e/demo-41415 open_ws: None
```

The checkout still exists with `open_workspace_id: None` — closed, not removed.

```bash
cd ~/.herdr/worktrees/herdr-e2e/demo-41415
git log --oneline -3
git status --short
cat hello.txt
```
...
level=INFO msg="herdr outcome" op=ensure-environment ticket=DEMO-41415 … result=created
level=INFO msg="herdr outcome" op=ensure-environment ticket=DEMO-41415 … result=exists
level=INFO msg="herdr outcome" op=find-terminal title=DEMO-41415:implement handle="" result=absent
level=INFO msg="herdr outcome" op=create-terminal envID=w2 title=DEMO-41415:implement result=ok
level=INFO msg="herdr outcome" op=ensure-terminal envID=w2 title=DEMO-41415:implement result=created
level=INFO msg="herdr outcome" op=find-terminal title=DEMO-41415:implement handle=w2:p2 result=found
level=INFO msg="herdr outcome" op=send-terminal title=DEMO-41415:implement handle=w2:p2 result=ok
level=INFO msg="herdr outcome" op=set-environment-status result=ok
level=INFO msg="herdr outcome" op=close-terminal title=DEMO-41415:implement handle=w2:p2 result=ok
level=INFO msg="herdr outcome" op=close-terminal title=DEMO-41415:verify handle=w2:p3 result=ok
level=INFO msg="herdr outcome" op=cleanup-run ticket=DEMO-41415 … result=ok
level=INFO msg="herdr outcome" op=close-terminals ticket=DEMO-41415 … closed=0 result=ok
```
