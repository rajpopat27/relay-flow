#!/usr/bin/env node
// relayflow npm wrapper: downloads the prebuilt Go binary for this
// platform from GitHub Releases (tag cli/v<version>), falling back to
// `go build` from the packaged source when no release asset matches
// or the network is unavailable. Cached in os.tmpdir()/relayflow/bin.
...
const bin = path.join(binDir, isWin ? "relayflow.exe" : "relayflow")
