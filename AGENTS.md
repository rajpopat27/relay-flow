...
## External Beads/Dolt server for this repository

The local `.workflow/workflow.yaml` uses the external Dolt SQL server and the
existing database at `/home/raj/.beads/relay-flow/dolt-data/relay_flow`. Do not
use `bd dolt start` for this repository: that starts a Beads-managed per-project
server using `.beads/dolt`, which is a different data directory and can expose an
empty or incorrect database.

Start the external server with this exact command:

```sh
config=/home/raj/.beads/relay-flow/dolt-data/config.yaml
log=/home/raj/.beads/relay-flow/dolt-server-external.log
pidfile=/home/raj/.beads/relay-flow/dolt-server.pid
nohup dolt sql-server --config "$config" >"$log" 2>&1 < /dev/null &
pid=$!
printf '%s\n' "$pid" > "$pidfile"
for _ in $(seq 1 30); do
  if ss -ltn 2>/dev/null | grep -q ':13307\b'; then break; fi
  sleep 0.2
done
```

Verify the external connection and database before starting relay-flow:

```sh
BEADS_DIR=/home/raj/.beads/relay-flow/.beads bd dolt test
BEADS_DIR=/home/raj/.beads/relay-flow/.beads bd list --limit 5 --json
```

`bd dolt status` may report that no Beads-managed server is running; use
`bd dolt test` because this server is externally started. Stop the external
server with `kill "$(cat /home/raj/.beads/relay-flow/dolt-server.pid)"` when
needed. Never run `bd bootstrap` or `bd init` for this workspace without first
confirming whether the existing Dolt database must be restored.
